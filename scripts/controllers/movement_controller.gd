...
var jump_gravity: float = 50 #fall_gravity
...
func _is_falling(fall_state: FallState):
	if abs(velocity.y) > fall_state.terminal_velocity:
		velocity.y += -fall_state.fall_gravity
	else:
		velocity.y = -fall_state.terminal_velocity

func _slide(_movement_state: MovementState):
